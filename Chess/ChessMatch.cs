...
namespace ChessGame.Chess
{
    internal class ChessMatch
    {
        ...
        public Piece ExecuteMove(Position originPos, Position finalPos)
        {
            ...
            // special move e passant
            if (piece is Pawn)
            {
                if (originPos.Column != finalPos.Column && capturedPiece == null)
                {
                    Position p;
                    if (piece.Color == Color.White)
                    {
                        p = new Position(finalPos.Row + 1, finalPos.Column);
                    }
                    else
                    {
                        p = new Position(finalPos.Row - 1, finalPos.Column);
        ...
        public void PerformPlay(Position initialPosition, Position finalPosition)
        {
            ...
            Piece piece = Board.Piece(finalPosition);

            // Special move promotion
            if (piece is Pawn)
            {
                if ((piece.Color == Color.White && finalPosition.Row == 0) || (piece.Color == Color.Black && finalPosition.Row == 7))
                {
                    piece = Board.RemovePiece(finalPosition);
                    Pieces.Remove(piece);
            ...
            if (piece is Pawn && (finalPosition.Row == piece.Position.Row + 2 || finalPosition.Row == piece.Position.Row - 2))
            {
        ...
        public void UndoMove(Position initialPosition, Position finalPosition, Piece capturedPiece)
        {
            ...
            if (piece is Pawn)
            {
                if (initialPosition.Column != finalPosition.Column && capturedPiece == PossibleEnPassant)
                {
                    ...
                    if (piece.Color == Color.White)
                    {
                        if (piece.Color == Color.White)
                        {
                            p = new Position(3, finalPosition.Column);
                        }
                        else
                        {
                            p = new Position(4, finalPosition.Column);
                        }

                        Board.PutPiece(pawn, p);
                    }
                }
            }
        }
